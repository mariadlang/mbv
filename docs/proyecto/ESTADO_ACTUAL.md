...
- La aplicación OAuth todavía no está verificada por Google. El flujo funciona para cuentas admitidas, pero un lanzamiento general requiere verificación de marca/scopes para retirar la advertencia y el límite de usuarios no verificados.
...
12. Google Calendar está operativo para cuentas admitidas y ya completó OAuth, selección, sync e importación real. Para un lanzamiento general faltan la verificación pública de Google OAuth —la pantalla actual muestra “app no verificada”— y la ejecución externa del mantenimiento periódico. Una prueba real de escritura debe hacerse con un evento controlado y autorización explícita.
...
Siguiente paso operativo para Calendar: preparar y enviar a Google la verificación de marca y scopes sensibles para retirar la advertencia y el límite de usuarios; después, ejecutar una prueba controlada de escritura Google ↔ MBV si se autoriza expresamente. Sites requiere además un scheduler HTTP autenticado para el mantenimiento periódico porque no ejecuta por sí solo el cron de Vercel. Después, resolver las demás dependencias externas del lanzamiento comercial —webhook/conciliación de Mercado Pago, condiciones comerciales, dominio personalizado y master vectorial— sin mezclarlas con el alcance P1 ni iniciar P2 por anticipado.

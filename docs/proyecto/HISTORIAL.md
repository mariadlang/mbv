...
- **Tipo de cambio:** cambio local sin commit; marca, UX writing, acceso, accesibilidad, analítica, QA y documentación.
...
- **Qué se hizo — trial y Premium:** `src/domain/access.ts` centralizó duración, capacidades y copy; el mismo horizonte local de tres meses se aplica a periodos y fechas de mes, semana, día y captura rápida, con validación adicional antes de persistir; los planes existentes fuera del horizonte se conservan en solo lectura con actividades históricas, tareas y eventos enlazados sin duplicar equivalentes; cinco años continúa como Premium; Feed Hub no se anuncia como accesible.
...
- **Evidencia:** working tree sobre `53221567be90c3dd2b9e9d47e1dce4be19891cd8`. **Commit final pendiente; no atribuir esta entrada a un hash todavía.**
- **Validación ejecutada:** ESLint, TypeScript, build de producción Next.js y build de Sites/Vinext aprobados; 20 archivos y 96 pruebas unitarias aprobadas; 12 pares de contraste aprobados; ocho screenshots P0 generados e inspeccionados. La suite Playwright final aprobó 66 casos, omitió 8 de forma intencional y no tuvo fallos. La migración analítica v2 fue aplicada y un segundo dry-run confirmó `Remote database is up to date`. El smoke de producción permanece pendiente del despliegue.
- **Estado de implementación:** candidato local en curso. **Push a `origin/main` y despliegue no ejecutados al abrir esta entrada.**
- **Pendientes o limitaciones:** crear commit, push, despliegue y smoke reales; implementar webhook/conciliación segura de Mercado Pago; definir las condiciones comerciales; adjuntar el dominio personalizado; y recibir el master vectorial aprobado.

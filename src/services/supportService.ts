...
export const supportService = {
  ...
  trackEvent(token: string, input: Parameters<typeof repository.trackEvent>[1], options?: { signal?: AbortSignal }) { const parsed = productEventSchema.parse({ ...input, metadata: sanitizeProductMetadata(input.metadata ?? {}, input.eventName) }); return repository.trackEvent(token, parsed, options); },
};

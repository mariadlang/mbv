import { z } from "zod";
import { clientProductEventNames } from "@/src/domain/productAnalytics";
...
export const productEventSchema = z.object({ eventName: z.enum(clientProductEventNames), feature: z.string().trim().min(2).max(60), sessionId: z.string().min(6).max(100), dedupeKey: z.string().min(6).max(180), occurredAt: z.string().datetime(), metadata: z.record(z.string(), z.unknown()).default({}) });

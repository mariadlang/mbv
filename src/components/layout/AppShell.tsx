...
export function AppShell({ children, userName, userAvatar, saving, theme, accessText, isSuperadmin = false, onQuickAdd, onNeedHelp, onSignOut }: { children: ReactNode; userName: string; userAvatar?: string; saving: boolean; theme: "light" | "rose" | "taupe"; accessText?: string; isSuperadmin?: boolean; onQuickAdd: () => void; onNeedHelp: () => void; onSignOut?: () => void }) {
  ...
  return <div className={`app-shell theme-${theme} ${sidebarCollapsed ? "app-shell--collapsed" : ""} ${isToday ? "app-shell--today" : ""}`} data-color-mode={colorMode}>
    ...
    <aside className="sidebar" aria-label={t("Navegación principal")}>
      <NavLink to="/app/dashboard" className="sidebar__brand" aria-label="My Best Version, inicio"><BrandMark compact={sidebarCollapsed} iconOnly={sidebarCollapsed} />{isToday && !sidebarCollapsed && <span className="sidebar-today-claim">Planea · Haz · Vive · Avanza</span>}</NavLink>
      <nav className="sidebar__nav">{desktopPrimaryItems.map(([href, label, Icon]) => <NavLink key={href} to={href} className={`nav-item ${isPrimaryActive(href, pathname) ? "nav-item--active" : ""}`} title={sidebarCollapsed ? t(label) : undefined}><Icon size={19} strokeWidth={1.7} aria-hidden="true" />{!sidebarCollapsed && <span>{t(label)}</span>}</NavLink>)}</nav>
      <div className="sidebar__footer">
        <nav className="sidebar__utility-nav" aria-label="Utilidades">{desktopUtilityItems.map(([href, label, Icon]) => <NavLink key={href} to={href} className={({ isActive }) => `nav-item ${isActive ? "nav-item--active" : ""}`} title={sidebarCollapsed ? t(label) : undefined}><Icon size={18} strokeWidth={1.7} aria-hidden="true" />{!sidebarCollapsed && <span>{t(label)}</span>}</NavLink>)}{isToday && onSignOut && <button className="nav-item sidebar-today-signout" type="button" onClick={onSignOut}><LogOut size={18} />{!sidebarCollapsed && <span>{t("Cerrar sesión")}</span>}</button>}{isSuperadmin && <NavLink className={({ isActive }) => `nav-item sidebar-admin-link ${isActive ? "nav-item--active" : ""}`} to="/platform" title={sidebarCollapsed ? "Superadmin" : undefined}><Sparkles size={18} />{!sidebarCollapsed && <span>Superadmin</span>}</NavLink>}</nav>
        ...
        <div className="sidebar__footer-actions">{!isToday && onSignOut && <button className="sidebar-signout" type="button" onClick={onSignOut} aria-label={t("Cerrar sesión")} title={t("Cerrar sesión")}><LogOut size={17} /></button>}<button className="collapse-button" type="button" onClick={() => setSidebarCollapsed(!sidebarCollapsed)} aria-expanded={!sidebarCollapsed} aria-label={sidebarCollapsed ? t("Expandir navegación") : t("Contraer navegación")}>{sidebarCollapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}</button></div>
      </div>
    </aside>

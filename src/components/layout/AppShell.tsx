...
import { BarChart3, Banknote, Bell, BookOpen, CalendarDays, ChevronLeft, ChevronRight, CircleHelp, Eye, Feather, Flag, HeartPulse, LayoutDashboard, Layers3, ListTodo, Menu, MoreHorizontal, MoonStar, Plus, Settings, Sun, Target } from "lucide-react";
...
const desktopGroups = [
  ...
  { label: "Reconocer", items: [["/app/progress", "Progreso", BarChart3], ["/app/journal", "Mi diario", Feather], ["/app/settings", "Ajustes", Settings], ["/app/help", "¿Necesitas ayuda?", BookOpen]] },
] as const;
const mobileItems = [["/app/dashboard", "Inicio", LayoutDashboard], ["/app/planning/weekly", "Semana", CalendarDays], ["/app/today", "Hoy", Flag], ["/app/habits", "Hábitos", HeartPulse], ["/app/more", "Más", MoreHorizontal]] as const;
...
export function AppShell({ children, userName, userAvatar, saving, theme, onQuickAdd, onNeedHelp }: { children: ReactNode; userName: string; userAvatar?: string; saving: boolean; theme: "light" | "rose" | "taupe"; onQuickAdd: () => void; onNeedHelp: () => void }) {
  ...
  return <div className={`app-shell theme-${theme} ${sidebarCollapsed ? "app-shell--collapsed" : ""}`} data-color-mode={colorMode}>
    ...
    <div className="app-main"><header className="topbar"><NavLink to="/app/more" className="topbar__mobile-brand" aria-label="Abrir más opciones"><Menu size={20} /><BrandMark compact /></NavLink><div className="topbar__utilities"><button type="button" onClick={onNeedHelp} aria-label="Necesito ayuda" title="¿Necesitas ayuda?"><CircleHelp size={18} /></button><button type="button" onClick={toggleColorMode} aria-label={colorMode === "light" ? "Activar modo oscuro" : "Activar modo claro"} aria-pressed={colorMode === "dark"}>{colorMode === "light" ? <MoonStar size={18} /> : <Sun size={18} />}</button><button type="button" aria-label="Notificaciones"><Bell size={19} /></button></div><div className="topbar__status" aria-live="polite"><span className={saving ? "saving-dot saving-dot--active" : "saving-dot"} />{saving ? "Guardando…" : "Guardado"}</div><div className="topbar__profile"><span>{userName}</span><span className="topbar-avatar">{avatar}</span></div></header><main id="main-content" className="page-content" key={pathname}>{children}</main></div>

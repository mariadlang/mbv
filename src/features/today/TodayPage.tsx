...
export function TodayPage({ planner, onQuickCapture, onNeedHelp }: { planner: PlannerController; onQuickCapture: (defaults: QuickCaptureDefaults) => void; onNeedHelp: () => void }) {
  ...
  const eventItems: TimelineItem[] = todayEvents.map((event) => ({ id: event.id, type: "events", title: event.title, detail: event.category, action: <button type="button" onClick={() => openDayComposer("event", event.id)} aria-label={`Editar ${event.title}`}><Pencil size={15} /></button> }));
  ...
  return <div className={`page-stack today-command-center ${minimumMode ? "is-minimum" : ""}`}>
    ...
    <section className="today-work-grid">
      ...
      <Card className="day-timeline-card"><header><h2>Agenda y tareas de hoy</h2><button type="button" className="today-text-action" onClick={() => onQuickCapture({ source: "today", date: todayKey })}><Plus size={15} /> Añadir</button></header><div className="timeline-filters" role="tablist" aria-label="Filtrar Mi día">{([['all','Todo'],['tasks','Tareas'],['training','Entrenamiento'],['meals','Comidas'],['events','Eventos']] as const).map(([id,label]) => <button type="button" role="tab" aria-selected={timelineFilter === id} className={timelineFilter === id ? "is-active" : ""} key={id} onClick={() => setTimelineFilter(id)}>{label}</button>)}</div>{dayComposer && <DayComposerForm draft={dayComposer} goals={snapshot.goals} projects={snapshot.projects} onChange={setDayComposer} onSubmit={saveDayItem} onClose={() => setDayComposer(null)} />}{visibleTimeline.length ? <div className="day-timeline-list">{(minimumMode ? visibleTimeline.slice(0, 1) : visibleTimeline).map((item) => <article key={`${item.type}-${item.id}`}><span className={`timeline-kind timeline-kind--${item.type}`}>{item.type === "tasks" ? <Check size={17} /> : item.type === "training" ? <Dumbbell size={17} /> : item.type === "meals" ? <Utensils size={17} /> : <CalendarDays size={17} />}</span><div><strong className={item.completed ? "is-complete" : ""}>{item.title}</strong><small>{item.detail}</small></div>{item.action}</article>)}</div> : <div className="today-timeline-empty"><CalendarDays size={20} aria-hidden="true" /><p>Aún no has añadido tareas, entrenamientos, comidas o eventos para hoy.</p></div>}<div className="today-timeline-add-menu" aria-label="Añadir a Mi día">{([['task','Tarea'],['priority','Prioridad'],['habit','Hábito'],['workout','Entrenamiento'],['meal','Comida'],['event','Evento']] as const).map(([kind, label]) => <button type="button" key={kind} onClick={() => kind === "task" ? onQuickCapture({ source: "today", date: todayKey }) : kind === "priority" ? onQuickCapture({ source: "today", date: todayKey, focusPriority: 1 }) : openDayComposer(kind)}><Plus size={14} /> {label}</button>)}</div></Card>
    ...
    {!minimumMode && <section className="today-closing-grid">
      ...
      <Card className="daily-close-card"><header><div><h2>Cierre del día</h2><p>Cuando termines…</p></div><Sparkles size={22} /></header><form onSubmit={saveClose}><label><span>¿Qué avancé hoy?</span><textarea rows={3} value={advancedToday} onChange={(event) => { setAdvancedToday(event.target.value); setCloseSaved(false); }} placeholder="Escribe aquí…" /></label><label><span>¿Qué quiero recordar de hoy?</span><textarea rows={3} value={rememberToday} onChange={(event) => { setRememberToday(event.target.value); setCloseSaved(false); }} placeholder="Escribe aquí…" /></label><Button type="submit" disabled={(!advancedToday.trim() && !rememberToday.trim()) || closeSaved}><Save size={16} /> {dailyCloseEntry ? "Actualizar mi cierre" : "Guardar mi cierre del día"}</Button>{closeSaved && <small role="status">Tu cierre quedó guardado en Mi diario.</small>}</form></Card>

      <Card className={`weekly-review-prompt ${completedWeeklyReview ? "is-complete" : ""}`}><h2>Revisión semanal</h2><div className="weekly-review-copy"><CalendarDays size={28} /><div><h3>{completedWeeklyReview ? "Revisión semanal completada" : "Tu semana está lista para ser revisada."}</h3><p>{completedWeeklyReview ? "Puedes volver a leer lo que aprendiste y las decisiones que elegiste." : "Tómate unos minutos —los que hoy te sirvan— para cerrar la semana."}</p></div></div><Link className="button button--secondary" to="/app/planning/weekly?reset=1">{completedWeeklyReview ? "Ver revisión" : "Iniciar revisión"}</Link></Card>
    </section>}

    <Card className="today-quote"><Quote size={20} /><p>La vida que quieres construir también nace de decisiones pequeñas.</p><small>My Best Version <Sparkles size={14} /></small></Card>
  </div>;
}
...
function DayComposerForm({ draft, goals, projects, onChange, onSubmit, onClose }: { draft: DayComposer; goals: Array<{ id: string; title: string; status: string }>; projects: Array<{ id: string; name: string; status: string }>; onChange: (draft: DayComposer) => void; onSubmit: (event: FormEvent) => void; onClose: () => void }) {
  ...
  const recurrenceOptions: Array<[HabitRecurrence, string]> = [["today", "Solo hoy"], ["daily", "Todos los días"], ["weekdays", "Días laborables"], ["custom", "Personalizar días"]];
  const dayOptions = [[1, "L"], [2, "M"], [3, "X"], [4, "J"], [5, "V"], [6, "S"], [0, "D"]] as const;
  return <form className="day-inline-composer" onSubmit={onSubmit}><header><strong>{draft.id ? "Editar" : "Añadir"} {labels[draft.kind]}</strong><button type="button" onClick={onClose} aria-label="Cerrar formulario"><X size={15} /></button></header><label><span>Nombre</span><input autoFocus required minLength={2} value={draft.title} onChange={(event) => onChange({ ...draft, title: event.target.value })} placeholder={`Nombre de ${labels[draft.kind]}`} /></label>{draft.kind === "priority" && <label><span>Posición</span><select required value={draft.priority} onChange={(event) => onChange({ ...draft, priority: event.target.value as DayComposer["priority"] })}><option value="">Elige una</option><option value="1">Prioridad 1</option><option value="2">Prioridad 2</option><option value="3">Prioridad 3</option></select></label>}{taskLike && <><label><span>Meta</span><select value={draft.goalId} onChange={(event) => onChange({ ...draft, goalId: event.target.value })}><option value="">Sin meta</option>{goals.filter((goal) => goal.status === "active").map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select></label><label><span>Proyecto</span><select value={draft.projectId} onChange={(event) => onChange({ ...draft, projectId: event.target.value })}><option value="">Sin proyecto</option>{projects.filter((project) => project.status === "active").map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label></>}{draft.kind === "habit" && <fieldset className="day-habit-recurrence"><legend>¿Cuándo quieres hacerlo?</legend><div>{recurrenceOptions.map(([value, label]) => <button type="button" key={value} className={draft.habitRecurrence === value ? "is-selected" : ""} aria-pressed={draft.habitRecurrence === value} onClick={() => onChange({ ...draft, habitRecurrence: value })}>{label}</button>)}</div>{draft.habitRecurrence === "custom" && <div className="day-picker">{dayOptions.map(([day, label]) => <button type="button" key={day} aria-pressed={draft.scheduledDays.includes(day)} className={draft.scheduledDays.includes(day) ? "is-selected" : ""} onClick={() => onChange({ ...draft, scheduledDays: draft.scheduledDays.includes(day) ? draft.scheduledDays.filter((item) => item !== day) : [...draft.scheduledDays, day] })}>{label}</button>)}</div>}</fieldset>}{draft.kind === "workout" && <label><span>Duración en minutos</span><input type="number" min="1" value={draft.duration} onChange={(event) => onChange({ ...draft, duration: event.target.value })} /></label>}{draft.kind === "meal" && <label><span>Calorías opcionales</span><input type="number" min="0" value={draft.calories} onChange={(event) => onChange({ ...draft, calories: event.target.value })} /></label>}<Button type="submit" size="sm" disabled={draft.kind === "habit" && draft.habitRecurrence === "custom" && !draft.scheduledDays.length}><Save size={15} /> Guardar en Mi día</Button></form>;
}

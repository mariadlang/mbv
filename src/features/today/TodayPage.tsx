...
import { CalendarDays, Check, ChevronRight, Circle, Dumbbell, GripVertical, Heart, HeartPulse, Lightbulb, Pencil, Plus, Quote, Save, Sparkles, Utensils } from "lucide-react";
...
export function TodayPage({ planner }: { planner: PlannerController }) {
  ...
  const workoutItems: TimelineItem[] = todayWorkout ? [{ id: todayWorkout.id, type: "training", title: todayWorkout.name ?? todayWorkout.goal ?? "Entrenamiento de hoy", detail: `${todayWorkout.exercises.length ? `${todayWorkout.exercises.length} ejercicios` : "Actividad general"}${todayWorkout.durationMinutes ? ` · ${todayWorkout.durationMinutes} min` : ""}`, action: <Link to={`/app/health?section=training&date=${todayKey}`}>Abrir</Link> }] : [];
  const mealItems: TimelineItem[] = (todayNutrition?.meals ?? []).map((meal) => ({ id: meal.id, type: "meals", title: meal.name, detail: `${meal.calories ?? 0} kcal`, completed: meal.completed, action: <Link to={`/app/health?date=${todayKey}`}>Ver</Link> }));

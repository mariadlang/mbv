...
import { getTrialPlanningDateBounds, isTrialPlanningDateAllowed, isTrialPlanningMonthAllowed, TRIAL_PLANNING_LIMIT_MESSAGE, type UserAccess } from "@/src/domain/access";
...
export function LifeHubPage({ planner, access, onQuickCapture }: { planner: PlannerController; access: UserAccess; onQuickCapture: (defaults: QuickCaptureDefaults) => void }) {
  ...
  const addEvent = async (event: FormEvent) => {
    ...
    if (!canPlanDate(eventDate)) { setPlanningError(TRIAL_PLANNING_LIMIT_MESSAGE); return; }
    setPlanningError("");
  ...
  const openItemEditor = (itemId: string) => {
    ...
    if (!item) return;
    if (item.tentativeDate && !canPlanTentativeDate(item.tentativeDate)) { setPlanningError(TRIAL_PLANNING_LIMIT_MESSAGE); return; }
    setPlanningError("");
  ...
  const saveItemEdit = async (event: FormEvent) => {
    ...
    if (editDraft.dateMode !== "flexible" && editDraft.date && !canPlanTentativeDate(editDraft.date)) { setPlanningError(TRIAL_PLANNING_LIMIT_MESSAGE); return; }
    setPlanningError("");
  ...
  const convertBrainItem = async (event: FormEvent) => {
    ...
    const date = conversionDestination === "monthly"
      ...
      : conversionDate;
    if (!canPlanDate(date)) { setPlanningError(TRIAL_PLANNING_LIMIT_MESSAGE); return; }
    setPlanningError("");
  ...
  const saveInboxEdit = async (event: FormEvent) => {
    ...
    if (inboxEdit.date && !canPlanDate(inboxEdit.date)) { setPlanningError(TRIAL_PLANNING_LIMIT_MESSAGE); return; }
    setPlanningError("");
  ...
  return (
    ...
    <div className="page-stack life-hub-page">
      ...
      {tab === "lists" && <>
        ...
        {inboxTasks.length > 0 && <Card className="brain-list brain-inbox-list"><header><div><h3>Sin clasificar</h3><small>{inboxTasks.length} en Bandeja</small></div></header>{inboxTasks.map((task) => { const readOnly = Boolean(task.date && !canPlanDate(task.date)); return editingInboxTaskId === task.id ? <form className="brain-item-editor" key={task.id} onSubmit={saveInboxEdit}><input autoFocus required value={inboxEdit.title} onChange={(event) => setInboxEdit({ ...inboxEdit, title: event.target.value })} aria-label="Editar captura" /><input type="date" min={dateBounds ? (today > dateBounds.min ? today : dateBounds.min) : today} max={dateBounds?.max} value={inboxEdit.date} onChange={(event) => { setPlanningError(""); setInboxEdit({ ...inboxEdit, date: event.target.value }); }} aria-label="Fecha de la captura" /><select value={inboxEdit.goalId} onChange={(event) => setInboxEdit({ ...inboxEdit, goalId: event.target.value })} aria-label="Meta"><option value="">Sin meta</option>{snapshot.goals.filter((goal) => goal.status === "active").map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select><select value={inboxEdit.projectId} onChange={(event) => setInboxEdit({ ...inboxEdit, projectId: event.target.value })} aria-label="Proyecto"><option value="">Sin proyecto</option>{snapshot.projects.filter((project) => project.status === "active").map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select><div><button type="submit"><Save size={14} /> Guardar</button><button type="button" onClick={() => setEditingInboxTaskId(null)}><X size={14} /> Cancelar</button></div></form> : <div className="brain-item" key={task.id}><button type="button" disabled={readOnly} title={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : undefined} onClick={() => void planner.toggleTask(task.id)} aria-label={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : `Completar ${task.title}`}><Circle size={14} /></button><div><strong>{task.title}</strong><small>Capturado · sin fecha</small></div><button type="button" disabled={readOnly} title={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : undefined} onClick={() => { setPlanningError(""); setEditingInboxTaskId(task.id); setInboxEdit({ title: task.title, date: task.date ?? "", goalId: task.goalId ?? "", projectId: task.projectId ?? "" }); }} aria-label={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : `Organizar ${task.title}`}><Pencil size={14} /></button></div>; })}</Card>}
        ...
        <Card className="brain-list brain-legacy-list"><header><div><h3>{listFilter === "all" ? "Ideas organizadas" : listLabels[listFilter]}</h3><small>{visibleBrainItems.filter((item) => item.status !== "completed" && item.status !== "released").length} abiertas</small></div></header>{visibleBrainItems.map((item) => { const readOnly = Boolean(item.tentativeDate && !canPlanTentativeDate(item.tentativeDate)); return editingItemId === item.id ? <form className="brain-item-editor" key={item.id} onSubmit={saveItemEdit}><input autoFocus value={editDraft.title} onChange={(event) => setEditDraft({ ...editDraft, title: event.target.value })} aria-label="Editar idea" /><select value={editDraft.type} onChange={(event) => setEditDraft({ ...editDraft, type: event.target.value as BrainDumpType })} aria-label="Mover a otra lista">{Object.entries(listLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><DateChoice mode={editDraft.dateMode} date={editDraft.date} dateBounds={dateBounds} label={item.title} onMode={(dateMode) => { setPlanningError(""); setEditDraft({ ...editDraft, dateMode, date: dateMode === "flexible" ? "" : editDraft.date }); }} onDate={(date) => { setPlanningError(""); setEditDraft({ ...editDraft, date }); }} /><select value={editDraft.goalId} onChange={(event) => setEditDraft({ ...editDraft, goalId: event.target.value })} aria-label="Meta de la idea"><option value="">Sin meta</option>{snapshot.goals.filter((goal) => goal.status === "active").map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select><select value={editDraft.projectId} onChange={(event) => setEditDraft({ ...editDraft, projectId: event.target.value })} aria-label="Proyecto de la idea"><option value="">Sin proyecto</option>{snapshot.projects.filter((project) => project.status === "active").map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select><div><button type="submit"><Save size={14} /> Guardar</button><button type="button" onClick={() => setEditingItemId(null)}><X size={14} /> Cancelar</button></div></form> : convertingItemId === item.id ? <form className="brain-item-editor brain-convert-editor" key={item.id} onSubmit={convertBrainItem}><strong>Convertir “{item.title}”</strong><label><span>Destino</span><select value={conversionDestination} onChange={(event) => { const destination = event.target.value as typeof conversionDestination; setPlanningError(""); setConversionDestination(destination); setConversionDate(destination === "monthly" ? today.slice(0, 7) : today); }}><option value="daily">Mi día</option><option value="weekly">Semana</option><option value="monthly">Mes</option></select></label><label><span>{conversionDestination === "monthly" ? "Mes" : "Día"}</span><input type={conversionDestination === "monthly" ? "month" : "date"} min={conversionDestination === "monthly" ? (monthBounds && today.slice(0, 7) < monthBounds.min ? monthBounds.min : today.slice(0, 7)) : (dateBounds && today < dateBounds.min ? dateBounds.min : today)} max={conversionDestination === "monthly" ? monthBounds?.max : dateBounds?.max} value={conversionDate} onChange={(event) => { setPlanningError(""); setConversionDate(event.target.value); }} /></label><small>Se conservará el mismo elemento y su vínculo de origen.</small><div><button type="submit"><Save size={14} /> Confirmar destino</button><button type="button" onClick={() => setConvertingItemId(null)}><X size={14} /> Cancelar</button></div></form> : <div className={`brain-item is-${item.status}`} key={item.id}><button type="button" disabled={readOnly} title={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : undefined} onClick={() => void planner.updateBrainDumpItem(item.id, { status: item.status === "completed" ? "idea" : "completed" })} aria-label={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : item.status === "completed" ? `Reabrir ${item.title}` : `Completar ${item.title}`}>{item.status === "completed" ? <Check size={14} /> : <Circle size={14} />}</button><div><strong>{item.title}</strong><small>{item.convertedTaskId ? `Convertido en ${item.destination === "monthly" ? "Mes" : item.destination === "weekly" ? "Semana" : "Mi día"}` : item.status === "planned" ? "Organizado" : "Capturado"} · {item.tentativeDate ? `${item.tentativeDate.length === 7 ? "Mes" : "Fecha"}: ${item.tentativeDate}` : "Flexible"}</small></div><button type="button" disabled={readOnly} title={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : undefined} onClick={() => openItemEditor(item.id)} aria-label={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : `Editar ${item.title}`}><Pencil size={14} /></button><button type="button" disabled={readOnly} title={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : undefined} onClick={() => { setPlanningError(""); setConvertingItemId(item.id); setConversionDestination("daily"); setConversionDate(today); }} aria-label={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : `Convertir ${item.title} en acción`}><CalendarDays size={14} /></button><button type="button" disabled={readOnly} title={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : undefined} onClick={() => void planner.updateBrainDumpItem(item.id, { status: "released" })} aria-label={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : `Liberar ${item.title}`}>×</button></div>; })}{!visibleBrainItems.length && <p className="brain-empty">Todavía no hay ideas organizadas en esta vista.</p>}</Card>
      </>}

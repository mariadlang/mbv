...
import { buildProgressEvidence, goalProgressSource } from "@/src/domain/progressEvidence";
  ...
  return (
    <div className="page-stack">
      ...
      <div className="metric-grid metric-grid--four">
        ...
        <Card className="metric-card"><span className="metric-card__icon"><CheckCircle2 size={20} /></span><p>Tareas completadas</p><strong>{evidence.completedTasks}</strong><small>Todo el periodo · fuente: tareas</small></Card>
        ...
        <Card className="metric-card"><span className="metric-card__icon"><Award size={20} /></span><p>Hitos alcanzados</p><strong>{evidence.completedMilestones}</strong><small>Todo el periodo · fuente: metas</small></Card>
      </div>
      ...
      <div className="progress-layout">
        ...
        {evidence.latestAchievement ? <Card className="achievement-card">
          ...
          <h2>{evidence.latestAchievement.title}</h2>
          <p>{evidence.latestAchievement.description}</p>
          <span className="achievement-card__date">{evidence.latestAchievement.condition}</span>
        </Card> : <Card className="progress-empty-state"><span className="achievement-card__icon"><Sparkles size={25} /></span><p className="eyebrow">Tu historia empieza aquí</p><h2>Todavía estamos construyendo tu historia</h2><p>Completa una tarea o registra un hábito para empezar a ver patrones reales.</p><Link className="button button--primary" to="/app/today">Ir a Mi día</Link></Card>}
      </div>
      ...
      <div className="progress-bottom-grid">
        ...
        <Card className="weekly-review-cta"><Sparkles size={24} /><p className="eyebrow">LO QUE ESTAMOS NOTANDO</p><h2>{evidence.completedTasks ? "Tus acciones ya están dejando evidencia." : "Tu ritmo puede empezar con una sola acción."}</h2><p>{evidence.completedTasks ? `Has completado ${evidence.completedTasks} tareas. Usa la revisión semanal para decidir cuáles sí merecen seguir.` : "Prepara una semana pequeña y posible; después podrás observar el patrón."}</p><Link className="button button--secondary" to="/app/planning/weekly?reset=1">Iniciar revisión semanal</Link></Card>
      </div>
    </div>
  );
}

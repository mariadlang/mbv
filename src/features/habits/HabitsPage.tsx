...
import { isTrialPlanningDateAllowed, TRIAL_PLANNING_LIMIT_MESSAGE, type UserAccess } from "@/src/domain/access";
    ...
    <div className="page-stack habits-page">
      ...
      {!activeHabits.length ? <Card className="habits-empty-card"><EmptyState title="Empieza con algo pequeño." text="Crea tu primer hábito y comienza a construir constancia." action={<Button onClick={openCreate}>Crear mi primer hábito</Button>} /></Card> : <div className="habits-dashboard-grid">
        ...
        <Card className="habit-week-card" id="habit-week">
          ...
          {weekHasReadOnlyDays && <div className="month-readonly-notice weekly-readonly-notice" role="status"><div><strong>{weekReadOnly ? "Semana en solo lectura" : "Algunos días están en solo lectura"}</strong><span>{TRIAL_PLANNING_LIMIT_MESSAGE}</span></div></div>}
          <div className="habit-week-table-wrap"><table className="habit-week-table"><thead><tr><th scope="col">Hábito</th>{weekDates.map((date) => <th scope="col" key={toLocalDateKey(date)} className={toLocalDateKey(date) === todayKey ? "is-today" : ""}><span>{["D", "L", "M", "X", "J", "V", "S"][date.getDay()]}</span><strong>{date.getDate()}</strong></th>)}</tr></thead><tbody>{activeHabits.map((habit) => <tr key={habit.id}><th scope="row">{habit.name}</th>{weekDates.map((date) => { const dateKey = toLocalDateKey(date); const scheduled = isHabitScheduledOn(habit, date); const log = snapshot.habitLogs.find((item) => item.habitId === habit.id && item.date === dateKey); const complete = isHabitLogComplete(habit, log); const readOnly = !canPlanDate(dateKey); return <td key={dateKey} className={dateKey === todayKey ? "is-today" : ""}>{scheduled ? <button type="button" className={complete ? "is-complete" : "is-pending"} disabled={readOnly} title={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : undefined} onClick={() => void planner.toggleHabit(habit.id, dateKey)} aria-label={readOnly ? "Disponible en solo lectura fuera del horizonte de la prueba" : `${habit.name}, ${dateKey}: ${complete ? "completado" : "aún no registrado"}`} aria-pressed={complete}>{complete ? <Check size={15} /> : <span />}</button> : <span className="habit-not-scheduled" aria-label="No programado">–</span>}</td>; })}</tr>)}</tbody></table></div>
          <footer className="habit-week-legend"><span><i className="is-complete"><Check size={12} /></i> Completado</span><span><i className="is-pending" /> Aún no registrado</span><span><i className="is-off">–</i> No programado</span></footer>
        </Card>

...
export function GoalsPage({ planner }: { planner: PlannerController }) {
  ...
  return <div className="page-stack">
    ...
    <Modal open={open} title={successGoalId ? "Meta creada" : "Crear una meta"} description={successGoalId ? "Ya sabes adónde quieres llegar. Ahora hagámoslo manejable." : "Empieza por el resultado y su razón. Los detalles técnicos pueden esperar."} onClose={closeCreate}>
      {successGoalId ? <div className="goal-success"><span><Check size={24} /></span><h2>¿Qué significaría avanzar este mes?</h2><p>Convierte la meta en una prioridad mensual y después elige qué podrías avanzar esta semana.</p><Link className="button button--primary" to="/app/planning" onClick={closeCreate}>Planificar esta meta <ArrowRight size={16} /></Link><Button variant="ghost" onClick={closeCreate}>Ahora no</Button></div> : <form className="form-grid goal-create-form" onSubmit={onSubmit}>

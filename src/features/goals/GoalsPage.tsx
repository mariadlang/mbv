...
export function GoalsPage({ planner }: { planner: PlannerController }) {
  ...
  const toggleProgressSettings = () => {
    if (!advancedOpen) {
      form.setValue("progressType", inferredMethod === "Valor numérico" ? "numeric" : inferredMethod === "Hitos sugeridos" ? "milestones" : "manual");
    }
  ...
  if (selectedGoal) {
    ...
    return <div className="page-stack goal-detail-page">
      ...
      <Card className="goal-next-step"><div><p className="eyebrow">Próximo paso</p><h2>{"title" in next ? next.title : "Elegir una acción concreta"}</h2><p>Haz visible lo siguiente antes de añadir más complejidad.</p></div><Link className="button button--primary" to="/app/planning">Planificar esta meta <ArrowRight size={16} /></Link></Card>
  ...
  return <div className="page-stack">
    ...
    <Modal open={open} title={successGoalId ? "Meta creada" : "Crear una meta"} description={successGoalId ? "Ya sabes adónde quieres llegar. Ahora hagámoslo manejable." : "Empieza por el resultado y su razón. Los detalles técnicos pueden esperar."} onClose={closeCreate}>
      {successGoalId ? <div className="goal-success"><span><Check size={24} /></span><h2>Tu meta ya tiene un camino</h2><p>El siguiente paso es darle un lugar en el mes. Después podrás llevar solo lo necesario a tu semana y a hoy.</p><div className="goal-cascade-path" aria-label="Camino desde la meta hasta hoy"><strong>Meta <Check size={14} /></strong><ArrowRight size={15} /><span>Mes</span><ArrowRight size={15} /><span>Semana</span><ArrowRight size={15} /><span>Hoy</span></div><Link className="button button--primary" to={`/app/planning?view=year&create=month&goal=${successGoalId}`} onClick={closeCreate}>Planificar esta meta <ArrowRight size={16} /></Link><Button variant="ghost" onClick={closeCreate}>Ahora no</Button></div> : <form className="form-grid goal-create-form" onSubmit={onSubmit}>

    ...
    {section === "nutrition" ? <>
      ...
      <Card className="fitness-day-card training-day-card">
        {selectedWorkout ? <>
          ...
          {selectedWorkout.exercises.length ? <div className="exercise-table">{selectedWorkout.exercises.map((exercise) => <article key={exercise.id}><div className="exercise-name"><span><Dumbbell size={18} /></span><div><h3>{exercise.name}</h3><small>{normalizedExerciseSets(exercise).length} series</small></div></div><div className="exercise-sets">{normalizedExerciseSets(exercise).map((set) => <div key={set.id}><small>Set {set.setNumber}</small><strong>{set.reps} rep</strong><span>{set.weight} kg</span></div>)}</div></article>)}</div> : <div className="workout-without-exercises"><Flame size={22} /><div><strong>Actividad sin series ni repeticiones</strong><p>Perfecto para cardio, caminatas, clases o deportes. La duración y el nombre son suficientes.</p></div></div>}

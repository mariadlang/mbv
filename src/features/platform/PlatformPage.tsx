...
const empty: PlatformData = { summary: {
  total_users:0,new_users_week:0,new_users_month:0,active_today:0,active_7d:0,weekly_active_users:0,active_30d:0,
  ...
  paywall_view_users:0,checkout_users:0,payment_users:0,paywall_to_checkout_rate:null,checkout_to_payment_rate:null,
  trial_users:0,trial_conversion_rate:null,renewal_eligible_users:0,renewal_users:0,renewal_rate:null,
  referral_visit_users:0,referral_signup_users:0,referral_activation_users:0,referral_signup_rate:null,referral_activation_rate:null,
  pending_suggestions:0,open_support:0,
}, users:[], usage:[], tickets:[], faqs:[], categories:[], settings:[], audit:[] };
...
function Summary({ data }: { data: PlatformData }) {
  const metrics = [["Usuarias registradas",data.summary.total_users],["Nuevas esta semana",data.summary.new_users_week],["Nuevas este mes",data.summary.new_users_month],["Activas hoy",data.summary.active_today],["WAU · activas 7 días",data.summary.weekly_active_users],["Activas · 30 días",data.summary.active_30d],["Onboarding · cohorte v2",formatRate(data.summary.onboarding_rate)],["Activación · cohorte v2",formatRate(data.summary.activation_rate)],["Retención D1 · usuarias/cohorte",formatCohortRate(data.summary.retention_1d,data.summary.retention_1d_users,data.summary.retention_1d_eligible_users)],["Retención D7 · usuarias/cohorte",formatCohortRate(data.summary.retention_7d,data.summary.retention_7d_users,data.summary.retention_7d_eligible_users)],["Retención D30 · usuarias/cohorte",formatCohortRate(data.summary.retention_30d,data.summary.retention_30d_users,data.summary.retention_30d_eligible_users)],["Sugerencias pendientes",data.summary.pending_suggestions],["Soporte abierto",data.summary.open_support]];
  const growthMetrics = [["Exportaron/iniciaron share · usuarias 7d",data.summary.share_users],["Exportaciones/inicios de share · 7d",data.summary.share_events],[`Share rate · ${data.summary.share_eligible_users} activas 7d`,formatRate(data.summary.share_rate)],["Recap semanal · usuarias 7d",data.summary.weekly_review_users],["Recaps completados · 7d",data.summary.weekly_review_events],[`Tasa de recap · ${data.summary.weekly_review_eligible_users} activas 7d`,formatRate(data.summary.weekly_review_rate)],["Paywall visto · usuarias",data.summary.paywall_view_users],["Checkout iniciado · usuarias",data.summary.checkout_users],["Pago confirmado · usuarias",data.summary.payment_users],["Paywall → checkout",formatRate(data.summary.paywall_to_checkout_rate)],["Checkout → pago",formatRate(data.summary.checkout_to_payment_rate,"Sin instrumentar pago")],[`Trial → pago · ${data.summary.trial_users} trials`,formatRate(data.summary.trial_conversion_rate,"Sin instrumentar pago")],[`Renovación · ${data.summary.renewal_eligible_users} elegibles`,formatRate(data.summary.renewal_rate,"Sin cohorte elegible")],["Renovaciones",data.summary.renewal_users],["Visitas referidas",data.summary.referral_visit_users],["Registros referidos",data.summary.referral_signup_users],["Activaciones referidas",data.summary.referral_activation_users],["Visita → registro",formatRate(data.summary.referral_signup_rate,"Sin visitas referidas")],["Visita → activación",formatRate(data.summary.referral_activation_rate,"Sin visitas referidas")]];
  const max = Math.max(1, ...data.usage.map((item) => item.event_count));
  return <><section className="platform-metrics">{metrics.map(([label,value]) => <Card key={label}><small>{label}</small><strong>{value}</strong></Card>)}</section><div className="platform-notice"><Activity size={20} /><p><strong>Base observable: {data.summary.analytics_v2_cohort_users} cuentas con eventos v2.</strong> Activación, retención, adopción y growth usan únicamente esta cohorte observable; no representa a todas las cuentas ni equivale a un registro de consentimiento persistido.</p></div><Card><h2>Growth · Etapa 1</h2><p>Métricas agregadas de revisión semanal, sharing y referrals; no muestran identificadores ni contenido personal.</p><section className="platform-metrics">{growthMetrics.map(([label,value]) => <Card key={label}><small>{label}</small><strong>{value}</strong></Card>)}</section></Card><Card className="platform-chart"><h2>Uso de funcionalidades</h2><p>Eventos v2 agregados, sin contenido escrito por las usuarias.</p>{data.usage.length ? data.usage.slice(0,10).map((item) => <div className="platform-bar" key={item.feature}><span>{item.feature}</span><i><b style={{ width:`${Math.max(3,(item.event_count/max)*100)}%` }} /></i><strong>{item.event_count}</strong></div>) : <Empty text="Todavía no hay eventos suficientes para mostrar tendencias." />}</Card></>;
}
...
function UsersView({ data, query, setQuery }: { data: PlatformData; query: string; setQuery(value: string): void }) {
  ...
  return <Card className="platform-table-card"><div className="platform-toolbar"><input aria-label="Buscar usuarias" placeholder="Buscar por nombre, correo o ID" value={query} onChange={(event) => setQuery(event.target.value)} /><span>{rows.length} resultados</span></div><div className="platform-table-wrap"><table><thead><tr><th>Usuaria</th><th>Registro</th><th>Última actividad</th><th>Idioma / zona</th><th>Onboarding</th><th>Activación v2</th><th>Sesiones</th><th>Metas</th><th>Tareas</th><th>Función principal</th><th>Marketing</th><th>Estado</th></tr></thead><tbody>{rows.map((user) => <tr key={user.user_id}><td data-no-translate="true" translate="no"><strong>{user.display_name || "Sin nombre"}</strong><small>{user.email}</small><small>{user.user_id.slice(0,8)}…</small></td><td>{formatDate(user.created_at)}</td><td>{formatDate(user.last_active_at)}</td><td data-no-translate="true" translate="no">{user.locale} · {user.timezone}</td><td>{user.onboarding_completed ? "Completo" : "Pendiente"}</td><td>{user.activated ? "Activada" : "En proceso"}</td><td>{user.session_count}</td><td>{user.goals_created}</td><td>{user.tasks_completed}</td><td>{user.top_feature || "—"}</td><td>{user.marketing_consent ? "Sí" : "No"}</td><td>{user.account_status}</td></tr>)}</tbody></table></div>{!rows.length && <Empty text="No hay usuarias que coincidan con la búsqueda." />}</Card>;
}

function UsageView({ data }: { data: PlatformData }) { return <Card className="platform-table-card"><h2>Adopción por funcionalidad</h2><p>La adopción usa como denominador sólo cuentas con eventos v2 observables. Activación v2: onboarding + una acción conectada + completar, registrar o reprogramar + una segunda sesión dentro de los primeros 7 días.</p><div className="platform-table-wrap"><table><thead><tr><th>Funcionalidad</th><th>Usuarias únicas</th><th>Eventos</th><th>Adopción v2</th><th>7 días</th><th>30 días</th><th>Último uso</th></tr></thead><tbody>{data.usage.map((item) => <tr key={item.feature}><td>{item.feature}</td><td>{item.unique_users}</td><td>{item.event_count}</td><td>{item.adoption_rate}%</td><td>{item.users_7d}</td><td>{item.users_30d}</td><td>{formatDate(item.last_used)}</td></tr>)}</tbody></table></div>{!data.usage.length && <Empty text="El tracking comenzará a poblar esta vista cuando las usuarias utilicen las funciones." />}</Card>; }

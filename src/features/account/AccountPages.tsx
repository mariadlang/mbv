...
export function LandingPage() {
  ...
  return <main className="marketing-page">
    ...
    <section className="hero-section"><div className="hero-copy"><p className="eyebrow">PLANEACIÓN PERSONAL · HÁBITOS · BIENESTAR</p><h1>Una vida más tuya,<br /><span>un paso posible a la vez.</span></h1><p>Convierte tu visión en decisiones claras para tus próximos años, meses, semanas y días, sin llenar tu agenda de culpa.</p><div className="hero-actions"><Link className="button button--primary" to="/signup">Comenzar prueba gratis <ArrowRight size={17} /></Link><Link className="button button--secondary" to="/trial">Ver qué incluye</Link></div><small><Check size={15} /> 15 días para explorar · sin pago automático</small></div><div className="hero-visual" aria-label="Vista previa del planner"><img src="/brand-icon.svg" alt="Logo de My Best Version" /><Card><span>Tu dirección</span><strong>De la visión a una semana posible</strong><div className="hero-progress"><i /><i /><i /></div></Card><Card><span>Mi día</span><strong>3 prioridades con espacio para respirar</strong></Card></div></section>
    <section id="como-funciona" className="marketing-section"><p className="eyebrow">CÓMO FUNCIONA</p><h2>Tu visión se convierte en un camino que sí puedes recorrer.</h2><div className="marketing-grid">{[["01","Mira hacia adelante","Define tu Vida soñada y tus horizontes de 3 años."],["02","Baja a lo concreto","Conecta año, trimestre, mes, semana y día."],["03","Observa con calma","Reconoce hábitos, energía y progreso sin castigos."]].map(([n,title,text]) => <Card key={n}><span>{n}</span><h3>{title}</h3><p>{text}</p></Card>)}</div></section>
...
export function PublicFrame({ children }: { children: React.ReactNode }) { return <main className="public-frame"><header><Link to="/"><BrandMark /></Link><nav><LanguageSwitcher compact /><Link to="/trial">Prueba</Link><Link to="/legal">Centro legal</Link><Link to="/privacy">Privacidad</Link><Link to="/terms">Términos</Link><Link to="/login">Iniciar sesión</Link></nav></header>{children}<footer className="public-legal-footer"><span>© 2026 My Best Version</span><Link to="/terms">Términos</Link><Link to="/data-policy">Tratamiento de datos</Link><Link to="/privacy">Privacidad</Link><Link to="/cookies">Cookies</Link><CookiePreferencesButton /><Link to="/pqr">PQR</Link><a href="https://www.sic.gov.co/" target="_blank" rel="noreferrer">Superintendencia de Industria y Comercio</a></footer></main>; }

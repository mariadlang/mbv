...
function LegalStatus() {
  return <>
    <p className="legal-page__updated">Versión {legalDocuments[0].version} · Vigente desde el 11 de septiembre de 2026</p>
...
export function TermsPage() {
  return <LegalArticle eyebrow="CONDICIONES DEL SERVICIO" title="Términos y Condiciones" lead="Estas condiciones regulan el uso de My Best Version en Colombia. No eliminan ni reducen los derechos irrenunciables de las personas consumidoras.">
    ...
    <section><h2>3. Cuenta y edad</h2><p>El servicio está dirigido a personas de 18 años o más. La persona usuaria debe entregar información correcta, proteger sus credenciales y comunicar accesos no autorizados. El inicio con Google solicita únicamente identidad básica autorizada. Google Calendar fue una autorización expresa e independiente, actualmente pausada, y nunca fue necesaria para usar la cuenta.</p></section>
    <section><h2>4. Contenido y almacenamiento</h2><p>El contenido detallado del planner pertenece a la persona usuaria y se guarda principalmente en IndexedDB del dispositivo. Las conexiones históricas de Google Calendar pudieron conservar separadamente una copia operativa de calendarios y eventos seleccionados; durante la pausa esa copia no se usa para sincronizar y puede eliminarse mediante una solicitud de supresión. Cambiar de navegador, dispositivo o dominio puede requerir exportar e importar un respaldo de los datos locales.</p></section>
    ...
    <section><h2>6. Gratis, beneficio Premium y pagos</h2><p>La cuenta comienza en el plan Gratis. Después de 30 días consecutivos de uso válido puede quedar elegible para una activación manual de 30 días de Premium; este beneficio no exige tarjeta, no genera cobros y no se convierte automáticamente en una suscripción. Premium también puede contratarse expresamente en modalidad mensual o anual. Antes de cualquier compra se presentan precio total, moneda, periodicidad, renovación, impuestos, proveedor de pago y mecanismo de cancelación. Los derechos de retracto, reversión y garantías legales no se renuncian.</p></section>
...
export function DataPolicyPage() {
  return <LegalArticle eyebrow="LEY 1581 DE 2012" title="Política de Tratamiento de Datos Personales" lead="Esta política describe las finalidades, bases, medidas y procedimientos aplicables al tratamiento de datos personales en My Best Version.">
    ...
    <section><h2>4. Finalidades</h2><p>Crear y autenticar cuentas; operar el planner; conservar preferencias; prestar soporte; proteger la seguridad; gestionar accesos, suscripciones y evidencias legales; responder solicitudes; cumplir obligaciones legales; y, únicamente con autorizaciones opcionales independientes, enviar comunicaciones comerciales o, cuando la integración esté disponible, sincronizar los calendarios y eventos que la persona elija entre Google Calendar y My Best Version.</p></section>
    <section><h2>5. Datos sensibles</h2><p>Los registros de salud, bienestar, ánimo, sueño, alimentación, medidas, journal y ciertas finanzas pueden revelar información sensible. Los títulos o descripciones de un calendario también podrían revelarla según lo que escriba la persona. Su entrega es facultativa. Solicitamos una autorización separada al entrar por primera vez a la función pertinente; negarla bloquea únicamente esa función o integración.</p></section>
    ...
    <section><h2>8. Conservación y eliminación</h2><p>Conservamos datos de cuenta mientras exista la relación y por el tiempo adicional necesario para obligaciones legales, seguridad o defensa de derechos. Los datos locales permanecen hasta que la persona los borre, limpie el navegador o importe un reemplazo. Durante la pausa de Google Calendar, las credenciales cifradas y la copia operativa histórica no se usan para sincronizar y se eliminan mediante una solicitud de supresión procedente o al suprimir la cuenta. La supresión se ejecuta cuando no exista deber legal o contractual de conservar.</p></section>
    ...
    <section><h2>10. Seguridad e incidentes</h2><p>Aplicamos control de acceso, sesiones gestionadas por Supabase, políticas RLS, minimización, almacenamiento local y respaldos bajo control de la persona. Los tokens de Google Calendar se cifran en el servidor, no se exponen al navegador ni se incluyen en respaldos; su almacenamiento y la copia de sincronización sólo son accesibles mediante procesos autenticados del servidor. Ningún sistema es infalible. Investigaremos incidentes, los documentaremos y notificaremos a las personas o autoridades cuando la ley lo exija.</p></section>
...
export function CookiesPage() {
  return <LegalArticle eyebrow="PREFERENCIAS" title="Política de Cookies y tecnologías similares" lead="Usamos almacenamiento del navegador para mantener la sesión, el idioma, la seguridad y los datos locales del planner.">
    <section><h2>Necesarias</h2><p>Son indispensables para autenticar, mantener preferencias esenciales, recordar la decisión de cookies y operar IndexedDB. Si se abre un enlace de invitación, localStorage puede conservar durante un máximo de 29 días únicamente su código opaco y fecha de llegada; no contiene identidad ni progreso de quien invita. Google puede usar sus propias tecnologías durante el inicio de sesión. La autorización independiente de Calendar está actualmente pausada; sus tokens históricos permanecen cifrados en el servidor y no se guardan en cookies ni almacenamiento local de My Best Version. Las tecnologías necesarias no pueden desactivarse desde el gestor mientras se use la aplicación.</p></section>
    ...
    <section><h2>Analítica y marketing</h2><p>No cargamos analítica publicitaria ni marketing de terceros. Analítica permanece apagada hasta que la autorices; con consentimiento, una cola local limitada transmite a Supabase eventos técnicos y metadata cerrada para medir funcionamiento, activación, retención y loops P2. Nunca incluye texto del planner, journal, bienestar, salud, finanzas, email ni contenido compartido. Marketing permanece apagado y exige una autorización independiente.</p></section>
...
export function PaymentsPage() {
  return <LegalArticle eyebrow="INFORMACIÓN COMERCIAL" title="Suscripciones, precios y pagos" lead="No se realiza ningún cobro sin una acción expresa. El beneficio temporal de Premium no exige tarjeta ni activa renovación automática por sí solo.">
    <section><h2>Información antes de comprar</h2><p>Premium ofrece las mismas funciones en dos modalidades: USD 2,99 al mes o USD 29,99 al año. Antes del pago se muestran precio total, moneda, impuestos aplicables, duración, periodicidad, renovación, cancelación, condiciones promocionales y datos del proveedor. Si una condición todavía no está definida o verificada, no se presenta como oferta disponible para cobrar.</p></section>
...
export function DataDeletionPage() {
  return <LegalArticle eyebrow="CONTROL DE TU INFORMACIÓN" title="Exportación y eliminación de datos" lead="Puedes administrar los datos locales desde Ajustes y solicitar la supresión de información de cuenta desde el Centro de Privacidad.">
    ...
    <section><h2>Datos históricos de Google Calendar</h2><p>La integración está temporalmente pausada y no realiza nuevas conexiones ni sincronizaciones. Puedes revocar el permiso desde tu Cuenta de Google y solicitar mediante el canal PQR o el Centro de Privacidad la eliminación de credenciales cifradas, calendarios vinculados y copia operativa histórica. Los eventos locales creados en My Best Version permanecen en el dispositivo.</p></section>
    <section><h2>Eliminar la cuenta</h2><p>Desde el Centro de Privacidad puedes crear una solicitud trazable. Verificaremos identidad, informaremos lo que deba conservarse legalmente y ejecutaremos la supresión procedente, incluidas las credenciales y la copia de Calendar asociadas a la cuenta. Cerrar sesión por sí solo no elimina datos ni desconecta integraciones.</p></section>
  </LegalArticle>;
}

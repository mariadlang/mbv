...
export function TermsPage() {
  return <LegalArticle eyebrow="CONDICIONES DEL SERVICIO" title="Términos y Condiciones" lead="Estas condiciones regulan el uso de My Best Version en Colombia. No eliminan ni reducen los derechos irrenunciables de las personas consumidoras.">
    ...
    <section><h2>3. Cuenta y edad</h2><p>El servicio está dirigido a personas de 18 años o más. La persona usuaria debe entregar información correcta, proteger sus credenciales y comunicar accesos no autorizados. El inicio con Google solicita únicamente identidad básica autorizada. La conexión opcional con Google Calendar requiere una autorización expresa e independiente y no es necesaria para usar la cuenta.</p></section>
    <section><h2>4. Contenido y almacenamiento</h2><p>El contenido detallado del planner pertenece a la persona usuaria y se guarda principalmente en IndexedDB del dispositivo. Si conecta Google Calendar, una copia operativa de los calendarios y eventos seleccionados se conserva separadamente en el servidor sólo para ejecutar y mostrar la sincronización. Cambiar de navegador, dispositivo o dominio puede requerir exportar e importar un respaldo de los datos locales.</p></section>
...
export function DataPolicyPage() {
  return <LegalArticle eyebrow="LEY 1581 DE 2012" title="Política de Tratamiento de Datos Personales" lead="Esta política describe las finalidades, bases, medidas y procedimientos aplicables al tratamiento de datos personales en My Best Version.">
    ...
    <section><h2>4. Finalidades</h2><p>Crear y autenticar cuentas; operar el planner; conservar preferencias; prestar soporte; proteger la seguridad; gestionar accesos, suscripciones y evidencias legales; responder solicitudes; cumplir obligaciones legales; y, únicamente con autorizaciones opcionales independientes, enviar comunicaciones comerciales o sincronizar los calendarios y eventos que la persona elija entre Google Calendar y My Best Version.</p></section>
    ...
    <section><h2>8. Conservación y eliminación</h2><p>Conservamos datos de cuenta mientras exista la relación y por el tiempo adicional necesario para obligaciones legales, seguridad o defensa de derechos. Los datos locales permanecen hasta que la persona los borre, limpie el navegador o importe un reemplazo. Las credenciales cifradas y la copia de sincronización de Google Calendar permanecen mientras la integración esté conectada y se eliminan del servidor al desconectarla desde la app o al suprimir la cuenta. La supresión se ejecuta cuando no exista deber legal o contractual de conservar.</p></section>
...
export function CookiesPage() {
  return <LegalArticle eyebrow="PREFERENCIAS" title="Política de Cookies y tecnologías similares" lead="Usamos almacenamiento del navegador para mantener la sesión, el idioma, la seguridad y los datos locales del planner.">
    <section><h2>Necesarias</h2><p>Son indispensables para autenticar, mantener preferencias esenciales, recordar la decisión de cookies y operar IndexedDB. Google puede usar sus propias tecnologías durante el inicio de sesión y, sólo si lo solicitas, durante la autorización independiente de Calendar. Los tokens de Calendar se cifran en el servidor y no se guardan en cookies ni almacenamiento local de My Best Version. Las tecnologías necesarias no pueden desactivarse desde el gestor mientras se use la aplicación.</p></section>
...
export function DataDeletionPage() {
  return <LegalArticle eyebrow="CONTROL DE TU INFORMACIÓN" title="Exportación y eliminación de datos" lead="Puedes administrar los datos locales desde Ajustes y solicitar la supresión de información de cuenta desde el Centro de Privacidad.">
    ...
    <section><h2>Desconectar Google Calendar</h2><p>Desde Ajustes puedes desconectar la integración en cualquier momento. La app intenta revocar el permiso en Google y elimina del servidor las credenciales cifradas, los calendarios vinculados y la copia operativa de los eventos. Los eventos creados localmente en My Best Version permanecen en el dispositivo. Revocar únicamente desde tu Cuenta de Google detiene accesos futuros, pero para eliminar la copia existente debes desconectar también desde la app o solicitar supresión.</p></section>

...
export function SettingsPage({ planner, onReplayTutorial, onRequestLogout, onLocalDataCleared }: { planner: PlannerController; onReplayTutorial: () => void; onRequestLogout: () => void; onLocalDataCleared: () => void }) {
  ...
  return <div className="page-stack settings-reference">
    ...
    <Card className="area-settings"><header><div><p className="eyebrow">Estructura transversal</p><h2>Áreas de vida</h2><p>Renombra, activa o reordena. Las metas y registros vinculados conservan su conexión.</p></div></header><div>{[...planner.snapshot.lifeAreas].sort((a,b) => a.order - b.order).map((area, index, all) => <div key={area.id}><input defaultValue={area.name} aria-label={`Nombre de ${area.name}`} data-no-translate="true" onBlur={(event) => planner.updateLifeAreaSettings(area.id, { name: event.target.value })}/><label><input type="checkbox" checked={area.active} onChange={(event) => planner.updateLifeAreaSettings(area.id, { active: event.target.checked })}/> Activa</label><button disabled={index === 0} aria-label={`Subir ${area.name}`} data-no-translate="true" onClick={() => planner.updateLifeAreaSettings(area.id, { direction: "up" })}><ArrowUp size={15}/></button><button disabled={index === all.length - 1} aria-label={`Bajar ${area.name}`} data-no-translate="true" onClick={() => planner.updateLifeAreaSettings(area.id, { direction: "down" })}><ArrowDown size={15}/></button></div>)}</div></Card>
    ...
    <Modal open={Boolean(preview)} title="Revisar respaldo antes de importar" description="Al confirmar, este contenido reemplazará los datos actuales del dispositivo." onClose={() => { setPreview(null); setPendingFile(null); }}>
      {preview && <div className="backup-preview"><div><strong data-no-translate="true">{preview.name}</strong><small>{m("settings.backup.exported", { date: formatDate(preview.exportedAt, { dateStyle: "medium", timeStyle: "short" }) })}</small></div><dl><div><dt>Áreas</dt><dd>{preview.areas}</dd></div><div><dt>Metas</dt><dd>{preview.goals}</dd></div><div><dt>Hábitos</dt><dd>{preview.habits}</dd></div><div><dt>Tareas</dt><dd>{preview.tasks}</dd></div><div><dt>Ingresos y gastos</dt><dd>{preview.transactions}</dd></div></dl>{preview.migrated && <Badge tone="warm">Se actualizará al formato v3</Badge>}<div className="modal__actions"><Button variant="ghost" onClick={() => { setPreview(null); setPendingFile(null); }}>Cancelar</Button><Button onClick={confirmImport}>Importar y reemplazar</Button></div></div>}
    </Modal>

...
export function FinancePage({ planner }: { planner: PlannerController }) {
  ...
  return <div className="finance-page page-stack">
    ...
    {tab === "summary" && <>
      ...
      <div className="finance-summary-grid">
        ...
        <Card className="finance-panel">
          ...
          {snapshot.savingsFunds.length ? <div className="finance-compact-list">{snapshot.savingsFunds.slice(0, 3).map((fund) => { const balance = calculateFundBalance(snapshot, fund.id); const progress = Math.min(100, Math.round(balance / fund.targetAmount * 100)); return <button key={fund.id} onClick={() => setTab("funds")}><div><strong data-no-translate="true">{fund.name}</strong><small>{money(balance)} de {money(fund.targetAmount)}</small></div><b>{progress}%</b></button>; })}</div> : <EmptyState title="Crea tu primer fondo" text="Ponle nombre al ahorro para conectarlo con una meta real." />}
        ...
        <Card className="finance-panel finance-next">
          ...
          {snapshot.recurringItems.filter((item) => item.active).sort((a, b) => a.dayOfMonth - b.dayOfMonth)[0] ? (() => { const item = snapshot.recurringItems.filter((entry) => entry.active).sort((a, b) => a.dayOfMonth - b.dayOfMonth)[0]; return <div className="next-commitment"><span>{item.dayOfMonth}</span><div><strong data-no-translate="true">{item.name}</strong><p>{money(item.amount)} · cada mes</p></div></div>; })() : <EmptyState title="Sin compromisos recurrentes" text="Añade solo los que quieras tener presentes." />}
    ...
    {tab === "budget" && <Card className="finance-form-card"><header><div><p className="eyebrow">Plan vs. real</p><h2>Presupuesto mensual · {currentMonth}</h2></div><Scale size={24} /></header><form onSubmit={saveBudget} className="finance-form-grid">
      ...
      {budgetCategories.map((category) => <label className="form-field" key={category.id}><span><span data-no-translate="true">{category.name}</span> · {category.type === "expense" ? "gasto" : category.type === "savings" ? "ahorro" : "deuda"}</span><input type="number" min="0" value={lineValues[category.id] ?? "0"} onChange={(event) => setLineValues({ ...lineValues, [category.id]: event.target.value })} /></label>)}
    ...
    {tab === "movements" && <div className="finance-two-column">
      <Card className="finance-form-card"><header><div><p className="eyebrow">Registro manual</p><h2>Nuevo movimiento</h2></div><Plus size={22} /></header><form className="finance-form-grid" onSubmit={submitMovement}>
        ...
        <label className="form-field"><span>Categoría</span><select value={movement.categoryId} onChange={(event) => setMovement({ ...movement, categoryId: event.target.value })}><option value="">Sin categoría</option>{snapshot.financeCategories.filter((item) => item.active).map((item) => <option key={item.id} value={item.id} data-no-translate="true">{item.name}</option>)}</select></label>
        <label className="form-field"><span>Cuenta o banco</span><select value={movement.accountId} onChange={(event) => setMovement({ ...movement, accountId: event.target.value })}><option value="">Sin cuenta</option>{snapshot.financialAccounts.filter((item) => item.status === "active").map((item) => <option key={item.id} value={item.id} data-no-translate="true">{item.name}</option>)}</select></label>
        {movement.type === "transfer" && <label className="form-field"><span>Cuenta destino</span><select value={movement.destinationAccountId} onChange={(event) => setMovement({ ...movement, destinationAccountId: event.target.value })}><option value="">Selecciona</option>{snapshot.financialAccounts.filter((item) => item.status === "active" && item.id !== movement.accountId).map((item) => <option key={item.id} value={item.id} data-no-translate="true">{item.name}</option>)}</select></label>}
        {(movement.type === "contribution" || movement.type === "withdrawal") && <label className="form-field"><span>Fondo</span><select aria-label="Fondo de ahorro" value={movement.fundId} onChange={(event) => setMovement({ ...movement, fundId: event.target.value })}><option value="">Selecciona</option>{snapshot.savingsFunds.map((item) => <option key={item.id} value={item.id} data-no-translate="true">{item.name}</option>)}</select></label>}
        {movement.type === "debt_payment" && <label className="form-field"><span>Deuda</span><select value={movement.debtId} onChange={(event) => setMovement({ ...movement, debtId: event.target.value })}><option value="">Selecciona</option>{snapshot.debts.map((item) => <option key={item.id} value={item.id} data-no-translate="true">{item.name}</option>)}</select></label>}
      ...
      <Card className="finance-panel"><header><div><p className="eyebrow">Detalle</p><h2>Ingresos y gastos del mes</h2></div><ReceiptText size={22} /></header><div className="movement-list">{snapshot.transactions.filter((item) => item.date.startsWith(currentMonth) && item.status === "active").map((item) => { const category = snapshot.financeCategories.find((entry) => entry.id === item.categoryId); const account = snapshot.financialAccounts.find((entry) => entry.id === item.accountId); const personalLabel = item.note || category?.name; return <div key={item.id}><span className={`movement-icon movement-icon--${item.type}`}>{item.type === "income" ? <ArrowUpRight size={15} /> : <ArrowDownRight size={15} />}</span><div>{personalLabel ? <strong data-no-translate="true">{personalLabel}</strong> : <strong>{transactionLabels[item.type]}</strong>}<small>{item.date} · {transactionLabels[item.type]} · <span data-no-translate={category || account ? "true" : undefined}>{category?.name ?? "Sin categoría"}{account ? ` · ${account.name}` : ""}</span></small></div><b>{money(item.amount)}</b></div>; })}{!snapshot.transactions.some((item) => item.date.startsWith(currentMonth)) && <EmptyState title="Aún no hay ingresos ni gastos" text="Registra el primero para comenzar tu resumen." />}</div></Card>
    </div>}

...
import { canAccessFeature, getTrialPlanningDateBounds, isTrialPlanningDateAllowed, isTrialPlanningMonthAllowed, TRIAL_PLANNING_LIMIT_MESSAGE } from "@/src/domain/access";
...
function taskProvenance(snapshot: PlannerSnapshot, task: Task): string {
  ...
  const monthlyResult = monthlyPlan ? `Resultado de ${format(monthDate(monthlyPlan.periodKey), "MMMM", { locale: es })}` : undefined;
  return [goal, monthlyResult, project].filter(Boolean).join(" · ") || "Bandeja";
}
...
export function PlanningPage({ planner, access, initialView = "year", onQuickCapture }: { planner: PlannerController; access: UserAccess; initialView?: PlanningView; onQuickCapture: (defaults: QuickCaptureDefaults) => void }) {
  ...
  const saveTaskEdit = async (event: FormEvent) => {
    ...
    if (taskEdit.date && !isTrialPlanningDateAllowed(access, taskEdit.date)) { setTaskError(TRIAL_PLANNING_LIMIT_MESSAGE); return; }
  ...
  const saveMonth = async (event: FormEvent) => {
    ...
    if (!isTrialPlanningMonthAllowed(access, periodKey)) {
      setMonthError("Tu prueba incluye un horizonte de 3 meses. Puedes conservar y consultar otros planes, pero necesitas Premium para modificarlos.");
      return;
    }
    ...
    if (datedEntries.some((item) => !isTrialPlanningDateAllowed(access, item.date))) {
      setMonthError(TRIAL_PLANNING_LIMIT_MESSAGE);
      return;
    }
  ...
  const saveReset = async () => {
    ...
    await planner.saveJournal(summary, { type: "monthly_reset", title: `Revisión mensual · ${format(anchorDate, "MMMM yyyy", { locale: es })}` });
  ...
  return <div className="page-stack cascade-page planning-v2">
    ...
    {view === "day" && <><PlanningSubheader title="Plan diario" description="Ordena lo esencial del día por fecha y prioridad." onBack={() => setView("year")} />{selectedDateReadOnly && <Card className="month-readonly-notice" role="status"><Lock size={20} aria-hidden="true" /><div><h2>Día en solo lectura</h2><p>{TRIAL_PLANNING_LIMIT_MESSAGE}</p></div></Card>}<Card className="daily-schedule-card daily-date-plan"><header><div><p className="eyebrow">Acciones del día</p><h2>{format(new Date(`${selectedDate}T12:00:00`), "EEEE d 'de' MMMM", { locale: es })}</h2></div><input type="date" aria-label="Fecha del plan diario" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} /></header>{!selectedDateReadOnly && <button type="button" className="task-manager-add task-manager-add--button" onClick={() => onQuickCapture({ source: "today", date: selectedDate })}><Plus size={16} /><span>Añadir una acción a este día</span><strong>Añadir</strong></button>}<div className="daily-priority-list">{snapshot.tasks.filter((task) => task.date === selectedDate && task.status !== "cancelled").sort((a, b) => (a.focusPriority ?? 9) - (b.focusPriority ?? 9)).map((task) => <div key={task.id} className={task.status === "completed" ? "is-complete" : ""}><button type="button" disabled={selectedDateReadOnly} onClick={() => planner.toggleTask(task.id)} aria-label={`${task.status === "completed" ? "Reabrir" : "Completar"} ${task.title}${selectedDateReadOnly ? ", solo lectura" : ""}`}>{task.status === "completed" ? <Check size={15} /> : <Circle size={15} />}</button><button type="button" disabled={selectedDateReadOnly} onClick={() => openTaskEditor(task)}><strong>{task.title}</strong><small>{taskProvenance(snapshot, task)}</small></button><button type="button" disabled={selectedDateReadOnly} onClick={() => openTaskEditor(task)} aria-label={`Editar ${task.title}${selectedDateReadOnly ? ", solo lectura" : ""}`}><Edit3 size={14} /></button></div>)}{!snapshot.tasks.some((task) => task.date === selectedDate && task.status !== "cancelled") && <EmptyState title={selectedDateReadOnly ? "No hay acciones en este día" : "Este día tiene espacio"} text={selectedDateReadOnly ? "Puedes consultarlo sin modificarlo durante la prueba." : "Añade una acción cuando sepas qué merece atención."} />}</div></Card></>}

    {view === "reset" && <><PlanningSubheader title="Revisión mensual" description="Cierra el ciclo con calma y decide qué quieres llevar al siguiente mes." onBack={() => setView("year")} /><div className="monthly-reset-grid"><Card className="reset-reflection"><RotateCcw size={22} /><p className="eyebrow">Revisión mensual · {format(anchorDate, "MMMM yyyy", { locale: es })}</p><h2>Cierra este ciclo y prepara el siguiente</h2><div className="monthly-reset-questions">{([['advanced','¿Qué avanzó?'],['learned','¿Qué aprendiste?'],['release','¿Qué quieres soltar?'],['adjust','¿Qué quieres ajustar?'],['next','¿Qué importa el próximo mes?']] as const).map(([key,label]) => <label key={key}><span>{label}</span><textarea required={key === "advanced"} rows={3} value={monthlyReset[key]} onChange={(event) => { setMonthlyReset({ ...monthlyReset, [key]: event.target.value }); setMonthlyResetSaved(false); }} /></label>)}</div><Button onClick={saveReset}><Save size={16} /> Guardar revisión mensual</Button>{monthlyResetSaved && <div className="reset-next-actions" role="status"><strong>Tu próximo ciclo está listo.</strong><Button onClick={() => openCreateMonth()}>Preparar mi próximo mes</Button><Button variant="secondary" onClick={() => setView("week")}>Preparar mi primera semana</Button></div>}</Card><div className="reset-summary"><Card className="brain-summary-card"><Sparkles size={23} /><div><p className="eyebrow">Lo que ya está en tu planner</p><strong>{snapshot.goals.filter((goal) => goal.status === "active").length} metas · {snapshot.habitLogs.length} registros de hábitos · {snapshot.journalEntries.length} páginas</strong><span>Finanzas, ánimo, pendientes y proyectos siguen disponibles para observar sin duplicarlos.</span></div></Card><Card className="brain-summary-card"><Sparkles size={23} /><div><p className="eyebrow">Resumen de tu Bandeja</p><strong>{brainSummary.captured} pensamientos capturados · {brainSummary.pending} abiertos</strong><span>{brainSummary.message}</span></div></Card><Card className="release-note"><p>Lo que elijo llevar al próximo mes</p><span>Conserva solo lo que todavía apoya tu visión y tus prioridades.</span></Card></div></div></>}
    ...
    <Modal open={monthModalOpen} title={`${editingMonthId ? "Editar" : "Planificar"} ${monthNames[Number(monthDraft.month) - 1]} ${monthDraft.year}`} description="Completa solo lo que hoy tenga sentido. Podrás editarlo después." onClose={() => setMonthModalOpen(false)}>
      <form className="form-grid month-create-form" onSubmit={saveMonth}>
        ...
        <label className="form-field form-field--full"><span>Meta activa (opcional)</span><select value={monthDraft.sourceGoalId} onChange={(event) => { const sourceGoalId = event.target.value; const goal = snapshot.goals.find((item) => item.id === sourceGoalId); setMonthDraft((current) => ({ ...current, sourceGoalId, focus: current.focus || goal?.title || "" })); }}><option value="">Sin meta vinculada</option>{snapshot.goals.filter((goal) => goal.status === "active").map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select></label>
        {monthDraft.sourceGoalId && <Card className="month-goal-context form-field--full"><Check size={17} /><div><p className="eyebrow">Meta que quieres acercar</p><strong>{snapshot.goals.find((goal) => goal.id === monthDraft.sourceGoalId)?.title}</strong><small>El resultado y las acciones conservarán esta conexión automáticamente.</small></div></Card>}
        <label className="form-field form-field--full"><span>Resultado concreto del mes</span><input value={monthDraft.focus} onChange={(event) => setMonthDraft({ ...monthDraft, focus: event.target.value })} placeholder="Ej. Completar el primer borrador" /></label>
        ...
        <fieldset className="form-field form-field--full month-entry-list"><legend>Acciones relacionadas (opcionales)</legend><small>Son las mismas tareas que verás en Bandeja, Semana y Mi día; no se crean copias.</small>{monthDraft.actions.map((item, index) => <div key={item.taskId ?? `action-${index}`}><input value={item.title} onChange={(event) => updateMonthEntry("actions", index, { title: event.target.value })} placeholder="Primera acción concreta" aria-label={`Acción ${index + 1}`} /><input type="date" min={trialDateBounds?.min} max={trialDateBounds?.max} value={item.date} onChange={(event) => updateMonthEntry("actions", index, { date: event.target.value })} aria-label={`Fecha opcional de la acción ${index + 1}`} /></div>)}<Button type="button" variant="ghost" onClick={() => addMonthEntry("actions")}><Plus size={15} /> Crear otra acción</Button></fieldset>
    ...
    <Modal open={Boolean(editingTaskId)} title="Editar acción" description="Los cambios se reflejan en Mes, Semana, Mi día y la meta vinculada." onClose={() => setEditingTaskId(null)}>
      <form className="form-grid" onSubmit={saveTaskEdit}>
        <label className="form-field form-field--full"><span>Acción</span><input autoFocus required minLength={2} value={taskEdit.title} onChange={(event) => setTaskEdit({ ...taskEdit, title: event.target.value })} /></label>
        <label className="form-field"><span>Fecha</span><input type="date" min={trialDateBounds?.min} max={trialDateBounds?.max} value={taskEdit.date} onChange={(event) => setTaskEdit({ ...taskEdit, date: event.target.value })} /></label>
        <label className="form-field"><span>Meta</span><select value={taskEdit.goalId} onChange={(event) => setTaskEdit({ ...taskEdit, goalId: event.target.value })}><option value="">Sin meta</option>{snapshot.goals.filter((goal) => goal.status === "active").map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select></label>
        ...
        {taskError && <p className="form-error form-field--full" role="alert">{taskError}</p>}
        <div className="modal__actions form-field--full"><Button type="button" variant="ghost" onClick={() => setEditingTaskId(null)}>Cancelar</Button><Button type="submit"><Save size={15} /> Guardar cambios</Button></div>
      </form>
    </Modal>
  </div>;
}
  ...
  return <>
    ...
    <section className="my-months-section" aria-labelledby="my-months-title">
      ...
      <div className="created-month-grid">{monthSlots.map((slot) => {
        ...
        if (!plan && !monthAllowed) return <Card id={`month-${slot.periodKey}`} className="created-month-card month-card--empty month-card--locked" key={slot.periodKey}><div className="month-card-main month-card-empty-action" aria-label={`${monthLabel} ${selectedYear}, disponible con Premium`}><header><div><span>{monthLabel}</span><strong>{selectedYear}</strong></div><Lock size={17} aria-hidden="true" /></header><div className="month-card-empty-copy"><Lock size={22} aria-hidden="true" /><h3>Disponible con Premium</h3><small>La prueba incluye un horizonte de 3 meses.</small><Link className="button button--outline" to="/upgrade">Desbloquear Premium</Link></div></div></Card>;
        if (!plan) return <Card id={`month-${slot.periodKey}`} className={`created-month-card month-card--empty ${slot.isCurrent ? "is-current" : ""}`} key={slot.periodKey}><button type="button" className="month-card-main month-card-empty-action" onClick={() => onAddPlan(slot.periodKey)} aria-label={`Añadir plan para ${monthLabel} ${selectedYear}`}><header><div><span>{monthLabel}</span><strong>{selectedYear}</strong></div>{slot.isCurrent ? <Badge tone="rose">Mes actual</Badge> : null}</header><div className="month-card-empty-copy"><CalendarDays size={22} /><h3>¿Qué quieres avanzar este mes?</h3>{monthIdeas.length ? <small>{monthIdeas.length} {monthIdeas.length === 1 ? "idea vinculada" : "ideas vinculadas"} desde Bandeja</small> : null}<span><Plus size={15} /> Añadir plan</span></div></button></Card>;
...
function ReadOnlyMonthDetail({ plan, snapshot, entries, todayKey, onBack, onPlanWeek }: { plan: CascadePlan; snapshot: PlannerSnapshot; entries: ReturnType<typeof collectMonthPlanEntries>; todayKey: string; onBack: () => void; onPlanWeek: () => void }) {
  ...
  const { actions, events } = entries;
  return <>
    <button type="button" className="back-link" onClick={onBack}><ArrowLeft size={16} /> Volver a Mis meses</button>
    <header className="month-detail-header">
      <div><p className="eyebrow">{monthState(plan, todayKey).label}</p><h1>{format(monthDate(plan.periodKey), "MMMM yyyy", { locale: es })}</h1><p>{plan.intention || "Un mes con espacio para decidir"}</p></div>
      <div><Badge tone="warm">Solo lectura</Badge><Link className="button button--outline" to="/upgrade">Desbloquear Premium</Link></div>
    </header>
    <Card className="month-readonly-notice"><Lock size={20} aria-hidden="true" /><div><h2>Tu plan se conserva sin cambios</h2><p>Este mes está fuera del horizonte de 3 meses incluido en la prueba. Puedes consultarlo aquí y editarlo con Premium.</p></div></Card>
    <div className="month-detail-grid">
      <Card className="month-focus-card"><p className="eyebrow">Enfoque del mes</p><h2>{plan.intention || "Todavía por definir"}</h2><div className="month-area-badges">{relatedAreas.map((area) => <Badge key={area.id} tone="warm">{area.name}</Badge>)}</div>{plan.objectives.length ? <ProgressBar value={monthProgress(plan)} label="Avance del mes" /> : null}</Card>
      <Card className="month-priority-card"><p className="eyebrow">Prioridades</p><h2>Lo que importa en este mes</h2>{plan.objectives.length ? <ol>{plan.objectives.map((priority) => <li key={priority}>{priority}</li>)}</ol> : <p>Aún no has elegido prioridades.</p>}</Card>
    </div>
    <div className="month-support-grid">
      <Card><p className="eyebrow">Acciones</p><h2>Pasos y actividades</h2>{actions.length ? <ul>{actions.map((action) => <li key={action.id}>{action.title}{action.date ? <time>{action.date}</time> : null}</li>)}</ul> : <p>No hay acciones definidas todavía.</p>}</Card>
      <Card><p className="eyebrow">Fechas y eventos</p><h2>Lo que ya está en calendario</h2>{events.length ? <ul>{events.map((event) => <li key={event.id}><CalendarDays size={15} /><span>{event.title}</span><time>{event.date}</time></li>)}</ul> : <p>Este mes no tiene eventos registrados.</p>}</Card>
    </div>
    <div className="planning-bridge"><div><p className="eyebrow">Siguiente nivel</p><h2>Tu semana sigue disponible</h2><p>Puedes organizar acciones de los próximos días sin modificar este plan mensual.</p></div><Button onClick={onPlanWeek}>Planificar mi semana <ChevronRight size={16} /></Button></div>
  </>;
}

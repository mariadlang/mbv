...
import { canAccessFeature, isTrialPlanningMonthAllowed } from "@/src/domain/access";
...
const emptyMonthDraft = (date = new Date()): MonthDraft => ({ month: String(date.getMonth() + 1), year: String(date.getFullYear()), focus: "", priorities: ["", "", ""], linkedPriority: "", areaIds: [], actions: "", status: "active" });
...
function monthState(plan: CascadePlan, todayKey: string) {
  ...
  if (plan.status === "closed" || plan.periodKey < currentMonth) return { label: "Cerrado", tone: "neutral" as const };
  if (plan.periodKey === currentMonth) return { label: "Mes actual", tone: "rose" as const };
  return { label: "Próximo", tone: "sage" as const };
}
...
export function PlanningPage({ planner, access, initialView = "year" }: { planner: PlannerController; access: UserAccess; initialView?: PlanningView }) {
  ...
  const visibleMonthPlans = monthPlans.filter((plan) => plan.periodKey.startsWith(`${selectedYear}-`));
  ...
  const availableYears = useMemo(() => Array.from(new Set([new Date().getFullYear() - 1, new Date().getFullYear(), new Date().getFullYear() + 1, ...monthPlans.map((plan) => Number(plan.periodKey.slice(0, 4)))] )).sort((a, b) => a - b), [monthPlans]);
  ...
  const openCreateMonth = () => {
    ...
    setMonthDraft(emptyMonthDraft(new Date(selectedYear, new Date().getMonth(), 1)));
    setMonthError("");
  ...
  const openEditMonth = (plan: CascadePlan) => {
    ...
    setMonthDraft({ month: String(Number(plan.periodKey.slice(5, 7))), year: plan.periodKey.slice(0, 4), focus: plan.intention, priorities: [...plan.objectives.slice(0, 3), "", ""].slice(0, 3), linkedPriority: plan.details?.linkedThreeYearPriority ?? "", areaIds: plan.areaIds ?? [], actions: plan.activities.map((activity) => activity.title).join("\n"), status: plan.status ?? "active" });
    setMonthError("");
    setMonthModalOpen(true);
  };
  ...
  const saveMonth = async (event: FormEvent) => {
    ...
    if (!isTrialPlanningMonthAllowed(access, periodKey)) {
      setMonthError("Durante la prueba puedes planificar el mes actual y los dos siguientes. Premium amplía este horizonte.");
      return;
    }
    ...
    const parsed = cascadePlanFormSchema.safeParse({ horizon: "monthly", periodKey, parentPlanId: threeYearPlan?.id, intention: monthDraft.focus, priority: priorities[0] ?? "", objectives: priorities, activities: monthDraft.actions.split("\n").map((item) => item.trim()).filter(Boolean).map((title) => ({ title, type: "activity" as const })), areaIds: monthDraft.areaIds, details: monthDraft.linkedPriority ? { linkedThreeYearPriority: monthDraft.linkedPriority } : {}, status: monthDraft.status });
  ...
  return <div className="page-stack cascade-page planning-v2">
    {view === "year" && <PlanningOverview access={access} snapshot={snapshot} fiveYearPlan={fiveYearPlan} threeYearPlan={threeYearPlan} visibleMonthPlans={visibleMonthPlans} selectedYear={selectedYear} availableYears={availableYears} todayKey={todayKey} onYearChange={setSelectedYear} onAddMonth={openCreateMonth} onEditLongTerm={openLongTerm} onOpenMonth={openMonthDetail} onEditMonth={openEditMonth} onOpenWeek={() => setView("week")} onOpenDay={() => setView("day")} onOpenReset={() => setView("reset")} />}
    ...
    <Modal open={monthModalOpen} title={editingMonthId ? "Editar mes" : "Agregar mes"} description="Crea un mes breve. Puedes completar los detalles cuando tengas más claridad." onClose={() => setMonthModalOpen(false)}><form className="form-grid month-create-form" onSubmit={saveMonth}><label className="form-field"><span>Mes</span><select value={monthDraft.month} disabled={Boolean(editingMonthId)} onChange={(event) => setMonthDraft({ ...monthDraft, month: event.target.value })}>{monthNames.map((name, index) => <option value={String(index + 1)} key={name}>{name}</option>)}</select></label><label className="form-field"><span>Año</span><input type="number" min={2020} max={2100} disabled={Boolean(editingMonthId)} value={monthDraft.year} onChange={(event) => setMonthDraft({ ...monthDraft, year: event.target.value })} /></label><label className="form-field form-field--full"><span>Enfoque del mes (opcional)</span><input value={monthDraft.focus} onChange={(event) => setMonthDraft({ ...monthDraft, focus: event.target.value })} placeholder="Ej. Construir una base sostenible" /></label><fieldset className="form-field form-field--full priority-stack"><legend>Prioridades del mes (1 a 3, opcionales)</legend>{monthDraft.priorities.map((value, index) => <input key={index} value={value} onChange={(event) => setMonthDraft({ ...monthDraft, priorities: monthDraft.priorities.map((item, itemIndex) => itemIndex === index ? event.target.value : item) })} placeholder={`Prioridad ${index + 1}`} />)}</fieldset>{threeYearPlan?.objectives.length ? <label className="form-field form-field--full"><span>Conectar con una prioridad a 3 años (opcional)</span><select value={monthDraft.linkedPriority} onChange={(event) => setMonthDraft({ ...monthDraft, linkedPriority: event.target.value })}><option value="">Sin conexión por ahora</option>{threeYearPlan.objectives.map((priority) => <option key={priority} value={priority}>{priority}</option>)}</select></label> : null}<fieldset className="form-field form-field--full month-area-selector"><legend>Áreas de vida relacionadas (opcionales)</legend>{snapshot.lifeAreas.filter((area) => area.active).map((area) => <label key={area.id}><input type="checkbox" checked={monthDraft.areaIds.includes(area.id)} onChange={() => setMonthDraft({ ...monthDraft, areaIds: monthDraft.areaIds.includes(area.id) ? monthDraft.areaIds.filter((id) => id !== area.id) : [...monthDraft.areaIds, area.id] })} /><span>{area.name}</span></label>)}</fieldset><label className="form-field form-field--full"><span>Acciones o eventos (uno por línea, opcional)</span><textarea rows={4} value={monthDraft.actions} onChange={(event) => setMonthDraft({ ...monthDraft, actions: event.target.value })} /></label>{editingMonthId && <label className="form-field form-field--full"><span>Estado</span><select value={monthDraft.status} onChange={(event) => setMonthDraft({ ...monthDraft, status: event.target.value as MonthDraft["status"] })}><option value="active">Activo</option><option value="draft">Borrador</option><option value="closed">Cerrado</option></select></label>}{monthError && <p className="form-error form-field--full" role="alert">{monthError}</p>}<div className="modal__actions form-field--full"><Button type="button" variant="ghost" onClick={() => setMonthModalOpen(false)}>Cancelar</Button><Button type="submit">{editingMonthId ? "Guardar cambios" : "Crear mes"}</Button></div></form></Modal>
  </div>;
}

function PlanningOverview({ access, snapshot, fiveYearPlan, threeYearPlan, visibleMonthPlans, selectedYear, availableYears, todayKey, onYearChange, onAddMonth, onEditLongTerm, onOpenMonth, onEditMonth, onOpenWeek, onOpenDay, onOpenReset }: { access: UserAccess; snapshot: PlannerSnapshot; fiveYearPlan?: CascadePlan; threeYearPlan?: CascadePlan; visibleMonthPlans: CascadePlan[]; selectedYear: number; availableYears: number[]; todayKey: string; onYearChange: (year: number) => void; onAddMonth: () => void; onEditLongTerm: (mode: LongTermMode) => void; onOpenMonth: (plan: CascadePlan) => void; onEditMonth: (plan: CascadePlan) => void; onOpenWeek: () => void; onOpenDay: () => void; onOpenReset: () => void }) {
  ...
  return <><SectionHeading eyebrow="Tu dirección" title="Planificación" description="Define hacia dónde quieres ir y convierte esa visión en decisiones para cada mes." action={<Button onClick={onAddMonth}><Plus size={17} /> Agregar mes</Button>} /><section className="planning-direction-grid" aria-label="Dirección de largo plazo">{canAccessFeature(access, "five_year_planning") ? fiveYearCard : <PremiumFeatureGate access={access} feature="five_year_planning" compact>{fiveYearCard}</PremiumFeatureGate>}<Card className="direction-card"><div className="direction-card__icon"><ListChecks size={21} /></div><div><p className="eyebrow">Puente estratégico</p><h2>Mis prioridades a 3 años</h2>{threeYearPlan?.intention ? <p>{threeYearPlan.intention}</p> : <p>Elige hasta tres prioridades que conviertan tu visión en una dirección clara.</p>}{threeYearPlan?.objectives.length ? <ol>{threeYearPlan.objectives.map((item) => <li key={item}>{item}</li>)}</ol> : null}</div><Button variant="secondary" onClick={() => onEditLongTerm("three")}><Edit3 size={15} /> {threeYearPlan ? "Editar" : "Agregar prioridades"}</Button></Card></section><section className="my-months-section"><header className="my-months-header"><div><p className="eyebrow">De la visión a la acción</p><h2>Mis meses</h2><p>Solo aparecen los meses que tú has creado.</p></div><div><label><span>Año</span><select value={selectedYear} onChange={(event) => onYearChange(Number(event.target.value))}>{availableYears.map((year) => <option key={year} value={year}>{year}</option>)}</select></label><Button variant="outline" onClick={onAddMonth}><Plus size={16} /> Agregar mes</Button></div></header>{visibleMonthPlans.length ? <div className="created-month-grid">{visibleMonthPlans.map((plan) => { const state = monthState(plan, todayKey); const progress = monthProgress(plan); const areas = snapshot.lifeAreas.filter((area) => plan.areaIds?.includes(area.id)); return <Card className="created-month-card" key={plan.id}><header><div><span>{format(monthDate(plan.periodKey), "MMMM", { locale: es })}</span><strong>{plan.periodKey.slice(0, 4)}</strong></div><Badge tone={state.tone}>{state.label}</Badge></header><h3>{plan.intention || "Un mes con espacio para decidir"}</h3>{plan.objectives.length ? <ol>{plan.objectives.map((priority) => <li key={priority}>{priority}</li>)}</ol> : <p className="muted-copy">Aún no has elegido prioridades.</p>}<div className="month-area-badges">{areas.slice(0, 3).map((area) => <Badge key={area.id} tone="warm">{area.name}</Badge>)}</div><ProgressBar value={progress} label="Avance de prioridades" /><footer><Button variant="ghost" onClick={() => onEditMonth(plan)}><Edit3 size={15} /> Editar</Button><Button onClick={() => onOpenMonth(plan)}>Ver mes <ChevronRight size={15} /></Button></footer></Card>; })}</div> : <Card className="months-empty"><EmptyState title="Aún no has creado ningún mes para este año" text="Agrega solo el mes que quieras planificar. No necesitas completar los doce meses de una vez." action={<Button onClick={onAddMonth}><Plus size={16} /> Crear mi primer mes</Button>} /></Card>}</section><section className="short-term-tools" aria-label="Herramientas de corto plazo"><div><p className="eyebrow">Cuando quieras bajar a lo concreto</p><h2>Continúa con tu ritmo real</h2></div><Button variant="secondary" onClick={onOpenWeek}>Plan semanal</Button><Button variant="secondary" onClick={onOpenDay}>Plan diario y horarios</Button><Button variant="ghost" onClick={onOpenReset}>Revisión mensual</Button></section></>;
}

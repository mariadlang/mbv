...
export function JournalPage({ planner }: { planner: PlannerController }) {
  ...
  return <div className="page-stack journal-reference">
    ...
    <div className="journal-tabs" role="tablist"><button className={tab === "entries" ? "is-active" : ""} onClick={() => setTab("entries")}>Entradas</button><button className={tab === "guided" ? "is-active" : ""} onClick={() => setTab("guided")}>Reflexiones guiadas</button><button className={tab === "reviews" ? "is-active" : ""} onClick={() => setTab("reviews")}>Revisiones</button></div>

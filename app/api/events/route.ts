...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!auth.e2e && auth.client) {
      const { error } = await auth.client.rpc("record_user_event", { next_event_name: parsed.eventName, next_feature: parsed.feature, next_session_id: parsed.sessionId, next_dedupe_key: parsed.dedupeKey, next_metadata: sanitizeProductMetadata(parsed.metadata, parsed.eventName), next_occurred_at: parsed.occurredAt });
